...
public class SlowDown extends SSprite {
	float x;
	float fromY;
	float toY;
	int round = 0;
	...
	OnHitListener mHitListener;
	
	public SlowDown(OnHitListener listener) {
		super(-100 * RM.SX, -100 * RM.SY, RM.txSlow);
		mHitListener = listener;
		if (Math.random() >= 0.5f) {
			upward = true;
		} else {
			upward = false;
